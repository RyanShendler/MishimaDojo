...
const TermListEntry = ({ termID, termName, termDescription }) => {
  ...
  return (
    <div className="flex flex-row shadow-md">
      {editing ? (
        <div className="flex w-3/4 flex-col items-center justify-center space-y-1 rounded-l-sm border border-black bg-[#EDF0F5] p-1">
          <input
        ...
        <div className="flex w-3/4 flex-col items-center justify-center rounded-l-sm border border-black bg-[#EDF0F5] p-1">
          <h3 className="text-center text-lg font-bold">{termName}</h3>
          <pre className="whitespace-pre-wrap text-center font-sans">
            {termDescription}
          </pre>
        </div>
      )}
